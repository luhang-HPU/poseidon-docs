...
namespace poseidon
{
    ...
    {
        if (poly_modulus_degree > POSEIDON_POLY_MOD_DEGREE_MAX || poly_modulus_degree < POSEIDON_POLY_MOD_DEGREE_MIN ||
        ...
        if (accumulate(
                bit_sizes.cbegin(), bit_sizes.cend(), POSEIDON_USER_MOD_BIT_COUNT_MIN,
                [](int a, int b) { return max(a, b); }) > POSEIDON_USER_MOD_BIT_COUNT_MAX ||
            accumulate(bit_sizes.cbegin(), bit_sizes.cend(), POSEIDON_USER_MOD_BIT_COUNT_MAX, [](int a, int b) {
                return min(a, b);
            }) < POSEIDON_USER_MOD_BIT_COUNT_MIN)
        {
            throw invalid_argument("bit_sizes is invalid");
        }
