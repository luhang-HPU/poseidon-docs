    ...
    namespace util
    {
        ...
        public:
...
#endif
                POSEIDON_ITERATE(iter(operand, modulus, result), coeff_modulus_size, [&](auto I) {
                ...
                PolyIter result) const
            {
                ...
                POSEIDON_ITERATE(iter(operand, result), size, [&](auto I) {
...
#endif
                POSEIDON_ITERATE(iter(operand, result), coeff_modulus_size, [&](auto I) {
            ...
            */
            POSEIDON_NODISCARD std::vector<std::uint32_t> get_elts_from_steps(const std::vector<int> &steps) const;
