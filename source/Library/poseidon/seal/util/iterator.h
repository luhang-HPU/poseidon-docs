...
#include "poseidon/seal/util/pointer.h"
#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <iostream>
...
namespace poseidon
{
    ...
    namespace util
    {
        ...
        In this file we define a set of custom iterator classes ("POSEIDON iterators") that are used throughout Microsoft
        POSEIDON for easier iteration over ciphertext polynomials, their RNS components, and the coefficients in the RNS
        components. All POSEIDON iterators satisfy the C++ LegacyRandomAccessIterator requirements. POSEIDON iterators are ideal
        to use with the POSEIDON_ITERATE macro, which expands to std::for_each_n in C++17 and to poseidon::util::seal_for_each_n
        in C++14. All POSEIDON iterators derive from POSEIDONIterBase.
        ...
        PtrIter<T *> and StrideIter<T *> are both templated POSEIDON iterators that wrap raw pointers. The difference
        ...
        @par IterTuple
        An extremely useful template class is the (variadic) IterTuple<...> that allows multiple POSEIDON iterators to be
        zipped together. An IterTuple is itself a POSEIDON iterator and nested IterTuple types are used commonly in the
        ...
        The individual components of an IterTuple can be accessed with the poseidon::util::get<i>(...) functions. The
        ...
        naturally be converted to POSEIDON iterators, outputs an appropriate iterator, or iterator tuple. Consider again the
        ...
        passing a variadic set of constructor arguments; iter will output an IterTuple consisting of POSEIDON iterators
        ...
        In addition to the iterator types described above, we provide ReverseIter<POSEIDONIter> that reverses the direction
        of iteration. ReverseIter<POSEIDONIter> dereferences to the same type as POSEIDONIter: for example, dereferencing
        ReverseIter<RNSIter> results in CoeffIter, not ReverseIter<CoeffIter>.

        It is easy to create a ReverseIter from a given POSEIDON iterator using the function reverse_iter. For example,
        ...
        POSEIDON iterators are made to be used with the POSEIDON_ITERATE macro to iterate over a certain number of steps, and
        for each step call a given lambda function. In C++17 POSEIDON_ITERATE expands to std::for_each_n, and in C++14 it
        expands to poseidon::util::seal_for_each_n -- a custom implementation. For example, the following snippet appears
        ...
        arguments and converts each RNS component separately. Looking at poseidon/seal/util/ntt.h we see that the function
        ntt_negacyclic_harvey_lazy is again implemented using POSEIDON_ITERATE. Specifically, it contains the following:
        ...
        There are two important coding conventions in the above code snippets that are to be observed:
            ...
            1. Use I, J, K, ... for the lambda function parameters representing POSEIDON iterators. This is compact and
               makes it very clear that the objects in question are POSEIDON iterators since such variable names should not
               be used in POSEIDON in any other context.
            2. Lambda functions passed to POSEIDON_ITERATE should almost always (see 3.) take a parameter of type auto. This
               will produce simple looking code that performs well with the expected outcome.
            3. The only exception to 2. is when POSEIDON_ITERATE operates on a single PtrIter<T *>: dereferencing returns a
               T &, which may be important to forward by reference to the lambda function. For an example of this, see
               poseidon::util::ntt_negacyclic_harvey in poseidon/seal/util/ntt.h, where the lambda function parameter is auto &.
        ...
        poseidon::util::negate_poly_coeffmod can negate a single RNS component modulo a given Modulus (CoeffIter overload),
        ...
        Sometimes inside POSEIDON_ITERATE lambda functions it is convenient to know the index of the iteration. This can be
        ...
        probably a good idea to call poseidon::util::allocate inside the lambda function for any allocations needed, rather
        ...
        */
        ...
        template <typename POSEIDONIter>
        class ReverseIter;

        template <typename... POSEIDONIters>
        class IterTuple;
        ...
        namespace iterator_internal
        {
            ...
            template <typename T>
            struct IterType<
                std::enable_if_t<std::is_base_of<POSEIDONIterBase, std::decay_t<T>>::value && !std::is_pointer<T>::value>,
                T>
            {
        ...
        public:
            ...
            POSEIDON_NODISCARD inline reference operator*() const noexcept
            {
                return value_;
            }
            ...
            template <typename SizeT>
            POSEIDON_NODISCARD inline self_type operator+(SizeT n) const noexcept
            {
                return value_ + static_cast<T>(n);
            }
            ...
            POSEIDON_NODISCARD inline operator T() const noexcept
            {
                return value_;
            }
        ...
        template <typename T>
        POSEIDON_NODISCARD inline auto seq_iter(T value = 0) -> SeqIter<T>
        {
            return value;
        }

        template <typename T>
        class PtrIter<T *> : public POSEIDONIterBase
        {
        public:
            ...
            template <typename SizeT>
            POSEIDON_NODISCARD inline reference operator[](SizeT n) const noexcept
            {
                return ptr_[n];
            }
            ...
            POSEIDON_NODISCARD inline pointer ptr() const noexcept
            {
                return ptr_;
            }
        ...
        template <typename T>
        class StrideIter<T *> : public POSEIDONIterBase
        {
        public:
            ...
            POSEIDON_NODISCARD inline std::size_t stride() const noexcept
            {
                return stride_;
            }
        ...
        public:
            ...
            template <typename SizeT>
            POSEIDON_NODISCARD inline value_type data(SizeT n) const
            {
                self_type result(*this);
...
//            //fixed me:
//            inline self_type operator+=(self_type &p)
//            {
//                if(this->coeff_modulus_size_ != p.coeff_modulus_size_)
//                {
//                     throw std::invalid_argument("poly +: rns num is different!");
//                }
//
//                return *this;
//            }
        ...
        template <typename POSEIDONIter>
        class ReverseIter : public POSEIDONIter
        {
        public:
            static_assert(
                std::is_base_of<POSEIDONIterBase, POSEIDONIter>::value,
        ...
        public:
            ...
            {
                static_assert(
                    sizeof...(Ts) == sizeof...(Rest) + 1, "std::tuple size does not match poseidon::util::IterTuple size");
            }
            ...
            POSEIDON_NODISCARD inline const POSEIDONIter &first() const noexcept
            {
                return first_;
            }

            POSEIDON_NODISCARD inline const IterTuple<Rest...> &rest() const noexcept
            {
                return rest_;
            }
        ...
        template <typename POSEIDONIter>
        class IterTuple<POSEIDONIter> : public POSEIDONIterBase
        {
        public:
            static_assert(
                std::is_base_of<POSEIDONIterBase, POSEIDONIter>::value,
        ...
        template <
            typename SizeT, typename POSEIDONIter,
        ...
        template <
            std::size_t N, std::size_t... Rest, typename... POSEIDONIters, typename = std::enable_if_t<sizeof...(Rest)>>
        POSEIDON_NODISCARD inline auto get(const IterTuple<POSEIDONIters...> &it) noexcept
        {
        ...
        template <typename... Ts>
        POSEIDON_NODISCARD inline auto iter(Ts &&... ts) noexcept -> typename iterator_internal::IterType<void, Ts...>::type
        {
