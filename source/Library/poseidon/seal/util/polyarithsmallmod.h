...
namespace poseidon
{
    namespace util
    {
            ...
            ConstRNSIter poly, std::size_t coeff_modulus_size, ConstModulusIter modulus, RNSIter result)
        {
            ...
            POSEIDON_ITERATE(iter(poly, modulus, result), coeff_modulus_size, [&](auto I) {
            ...
            ConstPolyIter poly_array, std::size_t size, ConstModulusIter modulus, PolyIter result)
        {
            ...
            POSEIDON_ITERATE(iter(poly_array, result), size, [&](auto I) {
            ...
            ConstCoeffIter poly, std::size_t coeff_count, const Modulus &modulus, CoeffIter result)
        {
            ...
            POSEIDON_ITERATE(iter(poly, result), coeff_count, [&](auto I) {
                auto coeff = get<0>(I);
#ifdef POSEIDON_DEBUG
...
#endif
            auto poly_modulus_degree = result.poly_modulus_degree();
            POSEIDON_ITERATE(iter(operand1, operand2, modulus, result), coeff_modulus_size, [&](auto I) {
...
#endif
            auto coeff_modulus_size = result.coeff_modulus_size();
            POSEIDON_ITERATE(iter(operand1, operand2, result), size, [&](auto I) {
        ...
        */
        ...
        inline void multiply_poly_scalar_coeffmod(
                ConstRNSIter poly, std::size_t coeff_count, std::uint64_t scalar, const Modulus &modulus,
                CoeffIter result)
        {
#ifdef POSEIDON_DEBUG
            if (!poly && coeff_modulus_size > 0)
...
#endif
            POSEIDON_ITERATE(iter(poly, result), 1, [&](auto I) {
...
#endif
            auto poly_modulus_degree = result.poly_modulus_degree();
            POSEIDON_ITERATE(iter(poly, mono_coeff, modulus, result), coeff_modulus_size, [&](auto I) {
                negacyclic_multiply_poly_mono_coeffmod(
