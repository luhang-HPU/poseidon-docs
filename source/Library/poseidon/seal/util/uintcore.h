...
namespace poseidon
{
    namespace util
    {
        POSEIDON_NODISCARD std::string uint_to_hex_string(const std::uint64_t *value, std::size_t uint64_count);

        POSEIDON_NODISCARD std::string uint_to_dec_string(
        ...
        inline void hex_string_to_uint_single(
            const char *hex_string, int char_count, std::size_t uint64_count, std::uint64_t *result)
        {
#ifdef POSEIDON_DEBUG
            if (!hex_string && char_count > 0)
        ...
        POSEIDON_NODISCARD inline bool is_equal_uint(
        ...
        POSEIDON_NODISCARD inline bool is_high_bit_set_uint(const std::uint64_t *value, std::size_t uint64_count)
        {
#ifdef POSEIDON_DEBUG
            if (!value)
            {
...
#pragma clang diagnostic ignored "-Wunused-parameter"
#endif
#endif
        POSEIDON_NODISCARD inline bool is_bit_set_uint(
            const std::uint64_t *value, std::size_t uint64_count POSEIDON_MAYBE_UNUSED, int bit_index)
        {
#ifdef POSEIDON_DEBUG
            if (!value)
            {
        ...
        inline void set_bit_uint(std::uint64_t *value, std::size_t uint64_count POSEIDON_MAYBE_UNUSED, int bit_index)
        {
#ifdef POSEIDON_DEBUG
            if (!value)
            {
...
#pragma clang diagnostic pop
#endif
#endif
        POSEIDON_NODISCARD inline int get_significant_bit_count_uint(const std::uint64_t *value, std::size_t uint64_count)
        {
#ifdef POSEIDON_DEBUG
            if (!value && uint64_count)
            {
        ...
        POSEIDON_NODISCARD inline std::size_t get_significant_uint64_count_uint(
        ...
        POSEIDON_NODISCARD inline std::size_t get_nonzero_uint64_count_uint(
            const std::uint64_t *value, std::size_t uint64_count)
        {
#ifdef POSEIDON_DEBUG
            if (!value && uint64_count)
        ...
        */
        ...
        POSEIDON_NODISCARD inline auto duplicate_uint_if_needed(
        ...
        POSEIDON_NODISCARD inline int compare_uint(
        ...
        POSEIDON_NODISCARD inline bool is_greater_than_uint(
            const std::uint64_t *operand1, const std::uint64_t *operand2, std::size_t uint64_count)
        {
            return compare_uint(operand1, operand2, uint64_count) > 0;
        }

        POSEIDON_NODISCARD inline bool is_greater_than_or_equal_uint(
            const std::uint64_t *operand1, const std::uint64_t *operand2, std::size_t uint64_count)
        {
            return compare_uint(operand1, operand2, uint64_count) >= 0;
        }

        POSEIDON_NODISCARD inline bool is_less_than_uint(
            const std::uint64_t *operand1, const std::uint64_t *operand2, std::size_t uint64_count)
        {
            return compare_uint(operand1, operand2, uint64_count) < 0;
        }

        POSEIDON_NODISCARD inline bool is_less_than_or_equal_uint(
            const std::uint64_t *operand1, const std::uint64_t *operand2, std::size_t uint64_count)
        {
            return compare_uint(operand1, operand2, uint64_count) <= 0;
        }
