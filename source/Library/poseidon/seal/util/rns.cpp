...
#include "poseidon/seal/util/uintarithmod.h"
#include "poseidon/seal/util/uintarithsmallmod.h"
#include <algorithm>
...
namespace poseidon
{
    namespace util
    {
        ...
        {
            bool result = true;
            POSEIDON_ITERATE(iter(base_), size_, [&](auto &I) { result = result && superbase.contains(I); });
            return result;
        }
        ...
        RNSBase RNSBase::extend(const Modulus &value) const
        {
            ...
            POSEIDON_ITERATE(iter(base_), size_, [&](auto I) {
                // The base must be coprime
        ...
        bool RNSBase::initialize()
        {
            ...
            if (size_ > 1)
            {
                ...
                POSEIDON_ITERATE(iter(base_, rnsbase_values), size_, [&](auto I) { get<1>(I) = get<0>(I).value(); });

                // Create punctured products
                StrideIter<uint64_t *> punctured_prod(punctured_prod_array_.get(), size_);
                POSEIDON_ITERATE(iter(punctured_prod, size_t(0)), size_, [&](auto I) {
                ...
                POSEIDON_ITERATE(iter(punctured_prod, base_, inv_punctured_prod_mod_base_array_), size_, [&](auto I) {
        ...
        void RNSBase::decompose(uint64_t *value, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                POSEIDON_ITERATE(iter(value, base_), size_, [&](auto I) {
        ...
        void RNSBase::decompose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                POSEIDON_ITERATE(iter(base_, value_out), size_, [&](auto I) {
                    // For each multi-precision integer in value_copy ...
                    POSEIDON_ITERATE(iter(get<1>(I), value_copy), count, [&](auto J) {
        ...
        void RNSBase::compose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                auto temp_mpi(allocate_uint(size_, pool));
                POSEIDON_ITERATE(iter(temp_array_iter, value_iter), count, [&](auto I) {
                    POSEIDON_ITERATE(
        ...
        void BaseConverter::fast_convert(ConstCoeffIter in, CoeffIter out, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ITERATE(iter(out, base_change_matrix_, obase_.base()), obase_size, [&](auto I) {
            ...
            POSEIDON_ITERATE(
                ...
                [&](auto I) {
                    ...
                    if (get<1>(I).operand == 1)
                    {
                        ...
                        POSEIDON_ITERATE(iter(get<0>(I), temp), count, [&](auto J) {
        ...
        void BaseConverter::exact_convert_array(ConstRNSIter in, CoeffIter out, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ITERATE(
                ...
                [&](auto I) {
                    ...
                    {
                        // No multiplication needed
                        POSEIDON_ITERATE(iter(get<0>(I), temp, v), count, [&](auto J) {
                            // Reduce modulo ibase element
                            get<1>(J)[ibase_index] = barrett_reduce_64(get<0>(J), get<2>(I));
            ...
            POSEIDON_ITERATE(iter(v, aggregated_rounded_v), count, [&](auto I) {
            ...
            POSEIDON_ITERATE(iter(out, temp, aggregated_rounded_v), count, [&](auto J) {
                // Compute the base conversion sum modulo obase element
        ...
        void BaseConverter::initialize()
        {
            ...
            POSEIDON_ITERATE(iter(base_change_matrix_, obase_.base()), obase_.size(), [&](auto I) {
                ...
                POSEIDON_ITERATE(iter(get<0>(I), ibase_punctured_prod_array), ibase_.size(), [&](auto J) {
        ...
        void RNSTool::initialize(size_t poly_modulus_degree, const RNSBase &q, const Modulus &t)
        {
            ...
            // primes q_i are bounded to be POSEIDON_USER_MOD_BIT_COUNT_MAX (60) bits, and all primes in B and m_sk are
            // POSEIDON_INTERNAL_MOD_BIT_COUNT (61) bits.
            ...
            if (32 + t_.bit_count() + total_coeff_bit_count >=
                POSEIDON_INTERNAL_MOD_BIT_COUNT * safe_cast<int>(base_q_size) + POSEIDON_INTERNAL_MOD_BIT_COUNT)
            {
                base_B_size++;
            }
            ...
            auto baseconv_primes =
                get_primes(mul_safe(size_t(4), coeff_count_), POSEIDON_INTERNAL_MOD_BIT_COUNT, base_Bsk_m_tilde_size);
            ...
            POSEIDON_ITERATE(iter(prod_B_mod_q_, base_q_->base()), base_q_size, [&](auto I) {
            ...
            POSEIDON_ITERATE(iter(inv_m_tilde_mod_Bsk_, base_Bsk_->base()), base_Bsk_size, [&](auto I) {
            ...
            POSEIDON_ITERATE(iter(prod_q_mod_Bsk_, base_Bsk_->base()), base_Bsk_size, [&](auto I) {
            ...
            if (base_t_gamma_)
            {
                ...
                POSEIDON_ITERATE(iter(prod_t_gamma_mod_q_, base_q_->base()), base_q_size, [&](auto I) {
                    get<0>(I).set(
                ...
                POSEIDON_ITERATE(iter(neg_inv_q_mod_t_gamma_, base_t_gamma_->base()), base_t_gamma_size, [&](auto I) {
            ...
            POSEIDON_ITERATE(iter(inv_q_last_mod_q_, base_q_->base()), base_q_size - 1, [&](auto I) {
        ...
        void RNSTool::divide_and_round_q_last_inplace(RNSIter input, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base()), base_q_size - 1, [&](auto I) {
                // (ct mod qk) mod qi
            ...
            RNSIter input, ConstNTTTablesIter rns_ntt_tables, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ALLOCATE_GET_COEFF_ITER(temp, coeff_count_, pool);
            POSEIDON_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base(), rns_ntt_tables), base_q_size - 1, [&](auto I) {
                // (ct mod qk) mod qi
                ...
                // Note: lambda function parameter must be passed by reference here
                POSEIDON_ITERATE(temp, coeff_count_, [&](auto &J) {
                ...
                POSEIDON_ITERATE(iter(get<0>(I), temp), coeff_count_, [&](auto J) { get<0>(J) += qi_lazy - get<1>(J); });
        ...
        void RNSTool::fastbconv_sk(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            POSEIDON_ITERATE(iter(alpha_sk, temp, input[base_B_size]), coeff_count_, [&](auto I) {
            ...
            POSEIDON_ITERATE(iter(prod_B_mod_q_, base_q_->base(), destination), base_q_size, [&](auto I) {
                ...
                POSEIDON_ITERATE(iter(alpha_sk, get<2>(I)), coeff_count_, [&](auto J) {
            ...
            */
            ...
            POSEIDON_ITERATE(
                ...
                [&](auto I) {
                    ...
                    POSEIDON_ITERATE(iter(get<0>(I), r_m_tilde, get<4>(I)), coeff_count_, [&](auto J) {
        ...
        void RNSTool::fast_floor(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            input += base_q_size;
            POSEIDON_ITERATE(iter(input, inv_prod_q_mod_Bsk_, base_Bsk_->base(), destination), base_Bsk_size, [&](auto I) {
                POSEIDON_ITERATE(iter(get<0>(I), get<3>(I)), coeff_count_, [&](auto J) {
        ...
        void RNSTool::decrypt_scale_and_round(ConstRNSIter input, CoeffIter destination, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ALLOCATE_GET_RNS_ITER(temp, coeff_count_, base_q_size, pool);
            POSEIDON_ITERATE(iter(input, prod_t_gamma_mod_q_, base_q_->base(), temp), base_q_size, [&](auto I) {
            ...
            // gamma inverse mod t
            POSEIDON_ITERATE(iter(temp_t_gamma[0], temp_t_gamma[1], destination), coeff_count_, [&](auto I) {
            ...
            RNSIter input, ConstNTTTablesIter rns_ntt_tables, MemoryPoolHandle pool) const
        {
            ...
            POSEIDON_ITERATE(iter(input, curr_modulus, inv_q_last_mod_q_, rns_ntt_tables), modulus_size - 1, [&](auto I) {
                ...
                POSEIDON_ITERATE(iter(delta_mod_q_i, c_last), coeff_count_, [&](auto J) {
                    get<0>(J) = add_uint_mod(get<0>(J), barrett_reduce_64(get<1>(J), get<1>(I)), get<1>(I));
                });
                ntt_negacyclic_harvey(delta_mod_q_i, get<3>(I));
                POSEIDON_ITERATE(iter(get<0>(I), delta_mod_q_i), coeff_count_, [&](auto J) {
