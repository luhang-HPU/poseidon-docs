...
namespace poseidon
{
    namespace util
    {
        ...
        public:
            ...
            POSEIDON_NODISCARD inline std::uint64_t get_root() const
            {
                return root_;
            }
            ...
            POSEIDON_NODISCARD inline MultiplyUIntModOperand get_from_inv_root_powers(std::size_t index) const
            {
#ifdef POSEIDON_DEBUG
                if (index >= coeff_count_)
                {
            ...
            POSEIDON_NODISCARD inline const Modulus &modulus() const
            {
                return modulus_;
            }

            POSEIDON_NODISCARD inline int coeff_count_power() const
            {
                return coeff_count_power_;
            }

            POSEIDON_NODISCARD inline std::size_t coeff_count() const
            {
                return coeff_count_;
            }
...
#endif
            POSEIDON_ITERATE(operand, size, [&](auto I) {
