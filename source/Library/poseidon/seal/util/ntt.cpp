...
#include "poseidon/seal/util/locks.h"
...
namespace intel
{
    namespace hexl
    {
            ...
            : public AllocatorInterface<NTT::AllocatorAdapter<seal::MemoryPoolHandle>>
        {
            ...
            {
                cache_.push_back(static_cast<poseidon::util::MemoryPool &>(handle_).get_for_byte_count(bytes_count));
            ...
            void deallocate_impl(void *p, POSEIDON_MAYBE_UNUSED std::size_t n)
            {
                auto it = std::remove_if(
                    ...
                    [p](const poseidon::util::Pointer<seal::seal_byte> &seal_pointer) { return p == seal_pointer.get(); });
        ...
        private:
            ...
            std::vector<poseidon::util::Pointer<seal::seal_byte>> cache_;
        };
            ...
            {
                {
                    ...
                    auto accessor = policy_.locker();
                    auto it = std::remove_if(
                        cache_.begin(), cache_.end(), [p](const poseidon::util::Pointer<seal::seal_byte> &seal_pointer) {
    ...
    namespace poseidon_ext
    {
        ...
        */
        static void compute_forward_ntt(
            poseidon::util::CoeffIter operand, std::size_t N, std::uint64_t modulus, std::uint64_t root,
...
namespace poseidon
{
    namespace util
    {
        ...
        void ntt_negacyclic_harvey(CoeffIter operand, const NTTTables &tables)
        {
            ...
            POSEIDON_ITERATE(operand, n, [&](auto &I) {
