...
#include "poseidon/seal/dynarray.h"
    ...
    */
    ...
    public:
        ...
        */
        ...
        @throws std::invalid_argument if pool is uninitialized
        */
        explicit Ciphertext(const PoseidonContext &context, MemoryPoolHandle pool = MemoryManager::GetPool())
            : data_(std::move(pool))
        {
        ...
        */
        explicit Ciphertext(
            const PoseidonContext &context, parms_id_type parms_id, MemoryPoolHandle pool = MemoryManager::GetPool())
            : data_(std::move(pool))
        {
            // Allocate memory but don't resize
        ...
        */
        explicit Ciphertext(
            const PoseidonContext &context, parms_id_type parms_id, std::size_t size_capacity,
        ...
        */
        POSEIDON_NODISCARD inline const auto &dyn_array() const noexcept
        {
            return data_;
        }
        ...
        */
        POSEIDON_NODISCARD inline ct_coeff_type *data() noexcept
        {
            return data_.begin();
        }
        ...
        */
        POSEIDON_NODISCARD inline const ct_coeff_type *data() const noexcept
        {
            return data_.cbegin();
        }
...
//        {
//
//            auto p = PolyIter(this->data(coeff_index), poly_modulus_degree_, coeff_modulus_size_);
//            return p;
//        }
        ...
        POSEIDON_NODISCARD inline const vector<RNSPoly> &polys() const{
            return  polys_;
        }

        POSEIDON_NODISCARD inline  vector<RNSPoly> &polys() {
            return  polys_;
        }
...
//        {
//
//            auto p = ConstPolyIter(this->data(coeff_index), poly_modulus_degree_, coeff_modulus_size_);
//            return p;
//        }
        ...
        @throws std::out_of_range if coeff_index is out of range
        */
//        POSEIDON_NODISCARD inline const ct_coeff_type &operator[](std::size_t coeff_index) const
//        {
//            return data_.at(coeff_index);
//        }
        ...
        */
        ...
        POSEIDON_NODISCARD inline std::size_t level() const noexcept
        {
            return coeff_modulus_size_ - 1;
        }
        ...
        */
        ...
        {
            return (
                !data_.size() || (size_ < POSEIDON_CIPHERTEXT_SIZE_MIN) ||
        ...
        */
//        POSEIDON_NODISCARD std::streamoff save_size(compr_mode_type compr_mode = Serialization::compr_mode_default) const;
...
//        }
        ...
        @throws std::runtime_error if I/O operations failed
        */
//        inline std::streamoff unsafe_load(const PoseidonContext &context, const seal_byte *in, std::size_t size)
//        {
//            using namespace std::placeholders;
...
//        }
        ...
        @throws std::runtime_error if I/O operations failed
        */
//        inline std::streamoff load(const PoseidonContext &context, const seal_byte *in, std::size_t size)
//        {
//            Ciphertext new_data(pool());
...
//        }
        ...
        */
        ...
        Returns whether the ciphertext is in NTT form.
        */
        POSEIDON_NODISCARD inline bool &is_ntt_form() noexcept
        {
            return is_ntt_form_;
        }
        ...
        */
        POSEIDON_NODISCARD inline parms_id_type &parms_id() noexcept
        {
            return parms_id_;
        }
        ...
        */
        POSEIDON_NODISCARD inline double &scale() noexcept
        {
            return scale_;
        }
        ...
        */
        POSEIDON_NODISCARD inline const double &scale() const noexcept
        {
            return scale_;
        }
        ...
        */
        ...
        void compute_ckks_hardware_id() const{
            if(polys_.empty()){
                throw invalid_argument("polys are empty");
            }
            for(auto &poly : polys_){
                poly.compute_ckks_hardware_id();
            }
        }

        inline bool &need_resize() {
            return need_resize_;
        }

        inline const bool &need_resize() const{
            return need_resize_;
        }
        /**
    ...
    private:
        ...
        //void expand_seed(const PoseidonContext &context, const UniformRandomGeneratorInfo &prng_info, SEALVersion version);

        //void save_members(std::ostream &stream) const;

        //void load_members(const PoseidonContext &context, std::istream &stream, SEALVersion version);
