...
namespace poseidon
{
    namespace
    {
            ...
            MemoryPool &pool)
        {
            ...
            POSEIDON_ITERATE(poly, coeff_count, [&](auto I) {
    ...
    Decryptor::Decryptor(const PoseidonContext &context, const SecretKey &secret_key) : context_(context)
    {
        // Verify parameters
...
//        }
    ...
    void Decryptor::dot_product_ct_sk_array(const Ciphertext &encrypted, RNSIter destination, MemoryPoolHandle pool)
    {
        ...
        size_t coeff_count = parms.degree();
        size_t coeff_modulus_size = coeff_modulus.size();
        size_t key_coeff_modulus_size = context_.crt_context()->key_context_data()->coeff_modulus().size();
        ...
        else
        {
            ...
            // Now do the dot product of encrypted_copy and the secret key array using NTT.
            // The secret key powers are already NTT transformed.
            POSEIDON_ALLOCATE_GET_POLY_ITER(encrypted_copy, encrypted_size - 1, coeff_count, coeff_modulus_size, pool);
            ...
            POSEIDON_ITERATE(iter(encrypted_copy, secret_key_array), encrypted_size - 1, [&](auto I) {
            ...
            POSEIDON_ITERATE(encrypted_copy, encrypted_size - 1, [&](auto I) {
