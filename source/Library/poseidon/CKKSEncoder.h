...
public:
    ...
    */
    ...
    inline void encode(
            ...
            MemoryPoolHandle pool = MemoryManager::GetPool()) const
    {
        encode(values, context_.crt_context()->first_parms_id(), scale, destination, std::move(pool));
    }
...
//    */
    ...
    {
        encode(value, context_.crt_context()->first_parms_id(), scale, destination, std::move(pool));
    }
//
//    /**
...
//#endif
    ...
    */
//    POSEIDON_NODISCARD inline std::size_t slot_count() const noexcept
//    {
//        return slots_;
//    }

    POSEIDON_NODISCARD inline const PoseidonContext &context() const{
        return context_;
    }
private:
    template <
            ...
            MemoryPoolHandle pool) const
    {
        ...
        int logn = static_cast<int>(parms.LogN());
        auto ntt_tables = global_context_data->small_ntt_tables();


        //auto &coeff_modulus_P = params->P;
...
//        if(context_.using_hardware() == true){
//            plain.compute_ckks_hardware_id();
//            CKKSMemoryManager_H* memoryManager = CKKSMemoryManager_H::getInstance();
//            auto &parms_id = plain.parms_id();
//            auto hardware_context = context_.hardware_context();
//            auto degree = plain.poly().poly_degree();
//            auto coeff_modulus_size = plain.poly().rns_num_total();
//            CKKSH
//            auto rns_max = api_.rns_max();
//
//            if(!result.is_valid()){
//                result.resize(context_,plain.parms_id(),util::mul_safe(degree, coeff_modulus_size));
//                result.poly().compute_ckks_hardware_id();
//            }
//
//            if(result.parms_id() != plain.parms_id()){
//                result.resize(context_,plain.parms_id(),plain.coeff_count());
//            }
//
//            if(result.poly().hardware_id() == 0)
//                result.poly().compute_ckks_hardware_id();
//
//            set<int> kept;
//            auto plain_addr = memoryManager->put_by_id_with_kept(context_,plain,plain.poly().hardware_id(),degree*rns_max*4/8*2, poseidon::ckks_ftt_inv_src, kept);
//            kept.insert(plain.poly().hardware_id());
//            auto wb_addr = memoryManager->put_by_id_with_kept_without_XDMA(context_,result, result.poly().hardware_id(), degree*rns_max*4/8*2,poseidon::ckks_ftt_inv_dst ,kept);
//            api_.ftt_inv(plain_addr, wb_addr,coeff_modulus_size);
//            if(result.parms_id() != parms_id){
//                result.resize(context_,parms_id,util::mul_safe(degree, coeff_modulus_size));
//            }
//            result.is_ntt_form() = false;
//            result.scale() = plain.scale();
//        }
//        else{
            for (std::size_t i = 0; i < coeff_modulus_size; i++)
            {
