...
namespace poseidon
{
    ...
    void BatchEncoder::populate_roots_of_unity_vector(const CrtContext &crtContext)
    {

        uint64_t root = crtContext.plain_ntt_tables()->get_root();
        auto &modulus = crtContext.first_context_data()->parms().plain_modulus();
    ...
    void BatchEncoder::encode(const vector<int64_t> &values_matrix, Plaintext &destination) const
    {
        ...
        if(values_matrix.empty()){
            POSEIDON_THROW(invalid_error,"values_matrix is empty");
        }
    ...
    void BatchEncoder::decode(const Plaintext &plain, vector<uint64_t> &destination, MemoryPoolHandle pool) const
    {
        ...
        if (!pool)
        {
            POSEIDON_THROW(invalid_error,"pool is uninitialized");
        }
    ...
    void BatchEncoder::decode(const Plaintext &plain, vector<int64_t> &destination, MemoryPoolHandle pool) const
    {
        if (!plain.is_valid())
        {
            POSEIDON_THROW(invalid_error,"plain is empty");
