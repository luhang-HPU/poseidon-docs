...
namespace poseidon
{
    Ciphertext &Ciphertext::operator=(const Ciphertext &assign)
    {
        ...
        // Then resize
        //resize( PoseidonContext &context, parms_id_type parms_id, size_t size)
        ...
        for(auto i = 0; i < assign.size_; ++i){
            polys_[i] = RNSPoly(crt_context_,data(i),parms_id_);
            polys_[i].set_hardware_id(assign.polys_[i].hardware_id());  //fixed me: this is hardware assign
        }
        // Size is guaranteed to be OK now so copy over
        //fixed me: only copy software part don't copy hardware data in cards


        return *this;
    }

    void Ciphertext::reserve(const PoseidonContext &context, parms_id_type parms_id, size_t size_capacity)
    {
        // Verify parameters
...
//        }
    ...
    {
        if (size_capacity < POSEIDON_CIPHERTEXT_SIZE_MIN || size_capacity > POSEIDON_CIPHERTEXT_SIZE_MAX)
        {
...
//        const PoseidonContext &context, const UniformRandomGeneratorInfo &prng_info, POSEIDONVersion version)
//    {
...
//        {
//            sample_poly_uniform_POSEIDON_3_4(prng, context_data_ptr->parms(), data(1));
//        }
//        else if (version.major == 3 && version.minor == 5)
//        {
//            sample_poly_uniform_POSEIDON_3_5(prng, context_data_ptr->parms(), data(1));
//        }
//        else
//        {
...
//    void Ciphertext::load_members(const PoseidonContext &context, istream &stream, POSEIDON_MAYBE_UNUSED POSEIDONVersion version)
//    {
//        // Verify parameters
//        if (!context.parameters_set())
//        {
...
//                    prng_info.scheme() = prng_type::blake2xb;
