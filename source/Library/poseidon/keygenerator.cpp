...
//        }
    ...
    KeyGenerator::KeyGenerator(const PoseidonContext &context, const SecretKey &secret_key) : context_(context)
    {
        // Verify parameters
//        if (!context_.parameters_set())
//        {
...
//        }
    ...
    void KeyGenerator::generate_sk(bool is_initialized)
    {
        ...
        if (!is_initialized)
        {
            ...
            sample_poly_ternary_with_hamming(context_.random_generator()->create(), context_,param_id, secret_key);
...
/*
    RelinKeys KeyGenerator::create_relin_keys(size_t count, bool save_seed)
    {
        ...
        if (!count || count > POSEIDON_CIPHERTEXT_SIZE_MAX - 2)
        {
...
*/
    ...
    KSwitchKeys
    KeyGenerator::create_switch_key(const SecretKey &prev_secret_key, const SecretKey &new_secret_key) const {
        if(!using_keyswitch_){
            throw invalid_argument("don't support switch key");
        }
    ...
    GaloisKeys KeyGenerator::create_galois_keys(const vector<std::uint32_t> &galois_elts, bool save_seed) const{
        if(!using_keyswitch_){
            throw invalid_argument("don't support switch key");
        }
...
/*
    void KeyGenerator::compute_secret_key_array(const PoseidonContext::ContextData &context_data, size_t max_power)
    {
#ifdef SEAL_DEBUG
        if (max_power < 1)
        {
