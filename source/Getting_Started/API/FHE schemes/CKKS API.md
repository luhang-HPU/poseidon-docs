...
For a message :math:`z \in \mathbb{C}^{\frac{N}{2}}` , the coefficients of plaintext polynomial $$m(X) = \sum_{i=0}^{N-1}m_iX^i$$ can be computed by $$m = (U;\overline{U})^T (z;\overline{z})= \frac{1}{N}(\overline{U}^T \cdot z + U^T \cdot \overline{z})$$  where $$U$$ is the $$(N/2) ×N$$ Vandermonde matrix generated by $${\zeta_j : 0 ≤ j < N/2}$$. 
